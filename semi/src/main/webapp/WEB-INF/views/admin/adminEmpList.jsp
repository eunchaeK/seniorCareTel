					...
					if(result.length>=1){
						result.forEach(function(item){
							...
							if(item.approval === true){
								str += "<td>O</td>";
								str += "<td><a href='${contextPath}/admin/empmod?number=" + item.number +"'>수정</a></td>";
							}else{
								str += "<td><a href='${contextPath}/admin/empapproval?number="+ item.number + "'>X</a></td>";
								str += "<td></td>";
							}
							/* str += "<td>" + item.approval + "</td>"; */ 
		...
		<div class="searchArea leftAlign">
			<form name="searchForm" autocomplete="on">
				<select id="searchType" name="type">
					...
					<option value="member_name">요양사이름</option>
					<option value="approval">승인여부</option>
				</select> <input type="search" name="keyword" id="searchKey"
					class="searchText" /> <input type="button" id="searchBtn"
					class="btn" value="검색" />
			</form>
		</div>

		<form action="/admin/empdelete" method="post" name="empsList">
			<table id="empTable">
				<thead>
					<tr id="tableHead">
				...
				<tbody>
					<c:forEach var="emp" items="${empsList}">
						<tr>
							...
							<td>${emp.phone}</td>
							<c:choose>
								<c:when test="${emp.approval eq true}">
									<td>O</td>
									<td><a
										href="${contextPath}/admin/empmod?number=${emp.number}">수정</a></td>
								</c:when>
								<c:otherwise>
									<td><a
										href="${contextPath}/admin/empapproval?number=${emp.number}">X</a></td>
									<td>&nbsp</td>
								</c:otherwise>
							</c:choose>
						</tr>
			...
			<div class="bottom">
				...
				<ul class="paging">
					<c:if test="${paging.prev}">
						<span><a
							href='<c:url value="/admin/memlist?page=${paging.startPage-1}"/>'>이전</a></span>
					</c:if>
					...
					<c:if test="${paging.next && paging.endPage>0}">
						<span><a
							href='<c:url value="/admin/memlist?page=${paging.endPage+1}"/>'>다음</a></span>
					</c:if>
				</ul>
			</div>
		</form>
